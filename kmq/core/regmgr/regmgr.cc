...
static mem_stat_t *rgm_mem_stats = &kmq_mem_stats.rgm;
static mem_stat_t *rgm_accepter_mem_stats = &kmq_mem_stats.rgm_accepter;
static mem_stat_t *rgm_register_mem_stats = &kmq_mem_stats.rgm_register;
static mem_stat_t *rgm_connector_mem_stats = &kmq_mem_stats.rgm_connector;
...
public:
    ...
    RgmHandler *Find(const struct kmqreg *rgh);
...
Rgm *NewRegisterManager(SpioConfig *_kmqconf) {
...
public:
    rgm_connector(const string &raddr, struct kmqreg *hdr, RgmHandler *h);
    ~rgm_connector();
...
int rgm_accepter::enable_event(Epoller *poller, int to_msec) {
    ...
    ee.to_nsec = (int64_t)to_msec * 1E6;
...
int rgm_accepter::handle_event(Epoller *poller, EpollEvent *ev, void *data) {
	...
	KMQLOG_ERROR("invalid epoll_events %d", ev->happened);
	return -1;
    }
    if (!(conn = listener->Accept())) {
	KMQLOG_ERROR("accept a null new connection");
	return EAGAIN;
    }
    ...
    if (reg->enable_event(poller, rgm->conf.registe_timeout_msec) < 0) {
	KMQLOG_ERROR("enable registe worker epollevent");
	delete reg;
	return EAGAIN;
    }
...
rgm_connector::rgm_connector(const string &_raddr, struct kmqreg *hdr, RgmHandler *h) :
...
int rgm_connector::tryconnect() {
	...
	if (internconn->Reconnect() < 0) {
	    KMQLOG_ERROR("appid: %s connect to remote app: %s", reghdr.appname, raddr.c_str());
	    return -1;
	}
...
int RegisterManager::Listen(const string &laddr) {
	...
	KMQLOG_ERROR("regmgr listen %s out of memory", laddr.c_str());
	return -1;
    }
    if (!(listen = ListenTCP("tcp", laddr, 1024))) {
	KMQLOG_ERROR("regmgr listen on %s with errno %d", laddr.c_str(), errno);
	return -1;
    }
    // Importance for fast restart
    listen->SetReuseAddr(true);
    if (!(acc = new (std::nothrow) rgm_accepter(listen))) {
	KMQLOG_ERROR("regmgr listen on %s out of memory", laddr.c_str());
	delete listen;
	return -1;
    }
...
int RegisterManager::push_tw_connector(rgm_connector *con) {
    timewait_connectors.Lock();
    timewait_connectors.PushTD(con, conf.registe_interval_msec);
    timewait_connectors.unLock();
    return 0;
}
...
RgmHandler *RegisterManager::Find(const struct kmqreg *rgh) {
...
int RegisterManager::process_rgm_registers() {
	...
	KMQLOG_ERROR("poller wait with errno %d", errno);
...
int RegisterManager::process_rgm_connectors() {
	...
	if (con->enable_event(poller, conf.registe_timeout_msec) == 0)
...
int RegisterManager::Start() {
    ...
    KMQLOG_NOTICE("regmgr start ok with pid %d", pid);
    while (!stopping) {
	process_rgm_registers();
	process_rgm_connectors();
    }
    KMQLOG_NOTICE("regmgr stop ok with pid %d", pid);    
    return 0;
}
