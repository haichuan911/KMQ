...
RgmHandler *rgmh_tb::balance_find(const struct kmqreg *rgh) {
    string appname;
	...
	KMQLOG_ERROR("invalid register appname: %s", rgh->appname);
	return NULL;
    }
    ksc_env.rgh = (struct kmqreg *)rgh;
    walkone(appname, __keepsession_check, &ksc_env);
    if (ksc_env.rgmh)
	return ksc_env.rgmh;

    pic_env.rgh = (struct kmqreg *)rgh;
