...
int AppCtx::EnableRegistry(Rgm *_rgm) {
    ...
    rgm = _rgm;
    if ((ret = _rgm->InsertHandler(&rom)) < 0)
	return ret;
...
int AppCtx::process_single_error(Role *r) {
	...
	if ((it = conf.at_apps.find(raddr)) != conf.at_apps.end()) {
	    ...
	    KMQLOG_NOTICE("appid %s remote node %s was removed", cid(), rip);
	}
    }
...
int AppCtx::init_r_stat_module_trigger_level(Role *r) {
    MQueue *mq = r->Queue();
...
int AppCtx::process_roles_register() {
	...
	if (r->InitResource(&mqp, poller, conf.role_timeout_msec) < 0) {
	    KMQLOG_ERROR("new role init resource errno: %d", errno);
	    continue;
	}
	init_r_stat_module_trigger_level(r);
	if (connect_mode == O_ACTIVE) {
	    if (conf.raddr_exist(r->cip()) == false) {
		process_single_error(r);
		continue;
	    }
	    if ((it = conf.wt_apps.find(r->cip())) != conf.wt_apps.end()) {
		conf.wt_apps.erase(it);
		conf.insert_active(r->cip(), r->Id());
	    }
	}
	if (IS_DISPATCHER(r->Type()))
	    lbp->add(r);
	...
	if (!list_empty(&err_roles)) {
	    first_no_dispatchers = rt_mstime();
	    process_roles_error();
	    KMQLOG_ERROR("close all receivers because of no dispatchers");
	}
    }
