...
public:
    ...
    int Register(struct kmqreg *header, Conn *conn);
    bool reg_keep_session(const struct kmqreg *reghdr);
...
bool my_rolemgr::reg_keep_session(const struct kmqreg *reghdr) {
...
int my_rolemgr::Register(struct kmqreg *header, Conn *conn) {
    string roleid;
    set<string>::iterator it;
    my_r r(header->rtype);
