    ...
    id(roleid), internconn(NULL), poller(NULL), mqueue(NULL), mqpool(NULL)
{
    ...
    icmp_snd_queue.Init(1E6);
    icmp_rcv_queue.Init(1E6);
...
int Role::HandleMsgev() {
	...
	KMQLOG_ERROR("%s %s(%s) EPOLLERR", cappid(), cip(), ROLESTR(this));
    } else if (ee.happened & EPOLLTIMEOUT) {
	KMQLOG_ERROR("%s %s(%s) EPOLLTIMEOUT", cappid(), cip(), ROLESTR(this));
    } else if (ee.happened & EPOLLRDHUP) {
	KMQLOG_ERROR("%s %s(%s) EPOLLRDHUP", cappid(), cip(), ROLESTR(this));
    }
...
int Role::PushSNDICMP(struct kmqmsg *icmp_msg) {
    return icmp_snd_queue.Push(icmp_msg);
}


struct kmqmsg *Role::PopSNDICMP() {
    return icmp_snd_queue.Pop();
}

int Role::PushRCVICMP(struct kmqmsg *icmp_msg) {
    return icmp_rcv_queue.Push(icmp_msg);
}


struct kmqmsg *Role::PopRCVICMP() {
...
int Role::InitResource(MQueuePool *mqp, Epoller *elpr, int to_msec) {
	...
	KMQLOG_ERROR("%s [%d %s %s] attach poller with errno %d",
		       cappid(), roletype, cid(), cip(), errno);
	return -1;
    }
    if (!(mqueue = mqp->Set(id))) {
	if (DetachPoller() < 0)
	    KMQLOG_ERROR("%s [%d %s %s] detach poller with errno %d",
			   cappid(), roletype, cid(), cip(), errno);
	return -1;
    }
    inited = 1;
