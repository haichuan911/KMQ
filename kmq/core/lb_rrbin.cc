...
static mem_stat_t *lb_rrbin_mem_stats = &kmq_mem_stats.lb_rrbin;
    ...
    idx(0), numbers(0), cap(1), backend_servers(NULL)
{
	...
	if (!(backend_servers = (Role **)mem_zalloc(cap * sizeof(Role *))))
	    KMQLOG_ERROR("create lb_rrbin with errno %d", errno);
...
int lb_rrbin::add(Role *r) {
    ...
    if (numbers >= cap) {
	KMQLOG_ERROR("realloc lb_rrbin with errno %d", errno);
	return -1;
    }
    KMQLOG_INFO("lb_rrbin add %s", r->cid());
...
int lb_rrbin::del(Role *r) {
	...
	if (numbers > 0 && idx >= numbers)
	    ...
	    idx = 0;
	KMQLOG_ERROR("lb_rrbin del %s", r->cid());
	return 0;
    }
    KMQLOG_ERROR("lb_rrbin del %s not found", r->cid());
    return -1;
}
...
Role *lb_rrbin::__loadbalance_send(struct kmqmsg *msg) {
    Role *r = NULL;
...
Role *lb_rrbin::loadbalance_send(struct kmqmsg *msg) {
    Role *r = NULL;
