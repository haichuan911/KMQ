...
mempool_t *
mp_create(uint32_t size)
{
    size = max(size, SLB_PAGESIZE);
    if (0 == size){
        return NULL;
    }
    mempool_t *p;
    p = (mempool_t *) mem_align(POOL_ALIGNMENT, size);
    if (p == NULL) {
        return NULL;
    }
...
void *
mp_alloc(mempool_t *pool, uint32_t size)
{
    int8_t      *m = NULL;
    mempool_t  *p = NULL;
    void *ptr = NULL;
...
void *
mp_nalloc(mempool_t *pool, uint32_t size)
{
    int8_t      *m = NULL;
    mempool_t  *p = NULL;
    void *ptr = NULL;
...
void *
mp_align(mempool_t *pool, uint32_t size, uint32_t alignment)
{
    void              *p;
    mempool_large_t  *large;
...
void *
mp_calloc(mempool_t *pool, uint32_t size)
{
    void *p;
