...
module_stat::module_stat(int key_range, stat_module_trigger *handler) :
...
#define __update_module_interval(t, interval, __key, __now_time)	\
    do {								\
	int idx = __key - 1;						\
...
int module_stat::update_one_key_stat(int key, int64_t cur_ms_time) {
    ...
    __update_module_interval(s, _S, key, cur_ms_time);
    __update_module_interval(m, _M, key, cur_ms_time);
    __update_module_interval(h, _H, key, cur_ms_time);
    __update_module_interval(d, _D, key, cur_ms_time);
