...
class my_module_trigger : public stat_module_trigger {
public:
    my_module_trigger() :
...
static int module_stat_non_trigger() {
...
static int module_stat_trigger() {
    my_module_trigger monitor;
    module_stat tstat(KEYRANGE, &monitor);
    int64_t start_tt = rt_mstime(), cur_tt = 0;
    ...
    KMQLOG_INFO("module stat _s trigger %d", monitor._s_trigger_cnt);
    KMQLOG_INFO("module stat _m trigger %d", monitor._m_trigger_cnt);
    KMQLOG_INFO("module stat _h trigger %d", monitor._h_trigger_cnt);
    KMQLOG_INFO("module stat _d trigger %d", monitor._d_trigger_cnt);
...
static int trigger_item_parse_test() {
    int tr = 0;
    int v = 0;
...
TEST(module_stat, non_trigger) {
    module_stat_non_trigger();
}


TEST(module_stat, trigger_parse) {
    trigger_item_parse_test();
}
