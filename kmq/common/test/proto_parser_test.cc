...
static int pp_client(void *arg_) {
	...
	KMQLOG_ERROR("DialTCP error %s", kmq_strerror(errno));
	return -1;
    }
    conn->SetSockOpt(SO_NONBLOCK, 1);
    conn->SetSockOpt(SO_WRITECACHE, 10);
...
static int test_proto_kmq_massage() {
    OSThread t[2];
...
TEST(proto, kmq_massage) {
    test_proto_kmq_massage();
}
...
// test api for proto_parser
//    int _recv_massage(Conn *conn, struct kmqhdr *hdr, int c, struct slice *s);
//    int _send_massage(Conn *conn, struct kmqhdr *hdr, int c, struct slice *s);
//    int _send_massage_async(Conn *conn, struct kmqhdr *hdr, int c, struct slice *s);
