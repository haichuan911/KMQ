...
int proto_parser::__recv_hdr(Conn *conn, struct kmqhdr *hdr) {
    int64_t nbytes = 0;
...
int proto_parser::__send_hdr(Conn *conn, const struct kmqhdr *hdr) {
    int64_t nbytes = 0;
    uint32_t size = RHDRLEN;
    struct kmqhdr chdr = *hdr;
...
int proto_parser::_recv_massage(Conn *conn, struct kmqmsg **header, int reserve) {
    int ret = 0;
    struct slice s = {};
    ...
    if (!bufhdr) {
	if (!(bufhdr = (struct kmqmsg *)mem_zalloc(MSGPKGLEN(&msg) + reserve))) {
	    errno = EAGAIN;
	    return -1;
	}
	if (msg.hdr.size)
...
int proto_parser::_send_massage(Conn *conn, struct kmqmsg *header) {
    int ret = 0;
...
int proto_parser::_send_massage_async(Conn *conn, struct kmqmsg *header) {
    int ret = 0;
    struct slice s = {};
...
int proto_parser::_recv_massage(Conn *conn, struct kmqhdr *hdr, struct slice *s) {
    int ret = 0;
    struct slice _s = {};
...
int proto_parser::_send_massage(Conn *conn, const struct kmqhdr *hdr, int c, struct slice *s) {
    int ret = 0;
...
int proto_parser::_send_massage_async(Conn *conn, const struct kmqhdr *hdr,
